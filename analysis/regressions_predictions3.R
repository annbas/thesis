setwd("~/YSPH/Thesis/data/")

#### install packages ####
# install.packages("pacman")
library(pacman)

p_load(plyr, tidyverse, Hmisc, e1071, lmtest, MASS, Metrics, BMA, ggpubr, Rcpp,
       mctest, MetBrewer, png, data.table, dplyr, hrbrthemes, reshape2)


source("../thesis_code/analysis/mod_functions.R")
...
ctedss.county.count <- ctedss.county.count %>%
                        group_by(county) %>%
                        filter(!is.na(county)) %>%
...
pred.dt.rate[, cor(x=covidnet.cases, y=pred), by = index]
...
preds.ci.county.rate <- merge(preds.ci.county.obs, county, all.x = TRUE, all.y = FALSE)

#add a total row (for the state)
ct.ci.tot <- data.frame(as.list(apply(preds.ci.county.rate[2:length(colnames(preds.ci.county.rate))], 2, sum)))
ct.ci.tot <- cbind(county = "Connecticut", ct.ci.tot)
#bind to dataset
preds.ci.county.rate <- rbind(preds.ci.county.rate, ct.ci.tot)

#calculate rates
preds.ci.county.rate <- preds.ci.county.rate %>%
  mutate('median.rate' = (median/tot.pop)*100000,
         'lcl.rate'  = (lcl/tot.pop)*100000,
         'ucl.rate'  = (ucl/tot.pop)*100000)

#export dataset for number plugging
write.csv(preds.ci.county.rate, file = "table2_data.csv", row.names = F)

#subtract ub, lb, and pt est from observed to get excess hospitalizations
...
#merge county ds to tot pop data
preds.ci.county.dev.rate <- merge(preds.ci.county.dev, county, all.x = TRUE, all.y = FALSE)

#calculate rates
preds.ci.county.dev.rate <- preds.ci.county.dev.rate %>%
                            mutate('median.rate' = (median.dev/tot.pop)*100000,
                                   'lcl.rate'    = (lcl.dev/tot.pop)*100000,
                                   'ucl.rate'    = (ucl.dev/tot.pop)*100000)

#create plot 
ggplot(preds.ci.county.dev.rate, aes(median.rate, county)) + 
...
ggplot(dph.pred, aes(x = predicted.rate, y = observed.rate)) + 
  ...
  geom_abline(slope = 1, linetype = "dotted", linewidth = 1, col = "red")+
  theme_bw()
